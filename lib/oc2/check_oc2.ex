defmodule Openc2.Oc2.CheckOc2 do
  @moduledoc """
  contains helper functions for decoding
  ...
  @action_target_pairs [
    ...
    {"query", "sbom"},
    {"query", "hello_world"},
    {"query", "x-sfractal-blinky:hello_world"},
    {"set", "led"},
    {"allow", "ipv4_net"},
    {"allow", "ipv6_net"},
    {"deny", "ipv4_net"},
  ...
  defp check_target(command) do
    if command.target in @targets do
      if {command.action, command.target} in @action_target_pairs do
        # valid so continue
        command
      else
